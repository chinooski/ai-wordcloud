...
async def generate(prompt: Prompt):
    ...
    try:
        ...
        # --- Exclude Words ---
        if prompt.exclude_words:
            exclude_list = [word.strip().lower() for word in prompt.exclude_words.split(',')]
        ...
        wc = WordCloud(width=800, height=800 if prompt.shape == "sphere" else 400, background_color="white", mode="RGB", mask=mask).generate(text)
